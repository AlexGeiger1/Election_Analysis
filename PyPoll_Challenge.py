...
with open(file_to_save, "w") as txt_file:
    ...
    for county in county_votes:
        ...
        county_voter_turnout = county_voter_turnout + county_votes[county]
        # 6c: Calculate the percentage of votes for the county.

        county_percentage =  float(county_voter_turnout)/ float(total_votes)  * 100
         # 6d: Print the county results to the terminal.
        county_results = (
        f"{county:}: {county_percentage:,.1f}% ({county_voter_turnout:,})\n")
        ...
        if county_voter_turnout > winning_county:
            winning_county = county_voter_turnout
